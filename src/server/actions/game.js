...
import { flushUpdate, createStagePiece, updateStage, createStage } from '../../shared/stage';
...
export const resStartGame = async (socket, data, redGame) => {
    ...
    try {
        if (!redGame.getGame(room).isOwner(name) || redGame.getGame(room).getStarted()) {
...
export const resChat = (socket, data, redGame) => {
    const { name, room, to, message } = data;
...
export const resGamePlayer = (socket, data, redGame) => {
    const { room, player, nbPiece, mallus } = data;
...
export const resUpdateCollision = (socket, data, redGame) => {
    const { name, room, score, level, lines, stage, loose } = data;
