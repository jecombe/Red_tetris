...
export const resLogin = (socket, data, redGame, callback) => {
    try {
        ...
        if (!name || !room || name.length > 15 || room.length > 15)
            throw new Error('Invalid name or room');
        if (redGame.getGame(room) && redGame.getGame(room).getStarted())
            throw new Error('Game already started');
        if (redGame.getGame(room) && redGame.getGame(room).getPlayer(name))
            throw new Error('Existing user with same name');
...
export const resLogout = (socket, data, redGame) => {
    try {
        ...
        if (!redGame.getGame(room).getPlayer(name))
            throw new Error("Player doesn't exist in this room");
